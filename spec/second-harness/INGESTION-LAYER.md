...
| G-07 | capability-token tenant isolation (§1-INV, the **security load**) — ⚠ **boilerplate landed (I8, PR #76), NOT wired**: `lgwks_capability.py` (`guard`/`make_tenant_filter`, hmac token) exists + tested, but `lgwks_vector.get_record`/`query_by_source` never filter on `tenant`, so A can read B today. Two-DB topology (shared world / private per-pair) + full gap map in [ARCH-two-db-multitenant.md](ARCH-two-db-multitenant.md) (L1/L2/L7); hardening in PLANS-NEXT-5.md. **Core to I8 now** (not deferred — multi-tenant concurrency is the real surface) | high (T0) |
| G-08 | CRDT layer (G-Set/OR-Set + cognition-clock) — ⚠ **module landed (I9, PR #76) but NOT deployed on the two stores**: `lgwks_crdt.py` (G-Set/OR-Set/LWW, SEC over 8 permutations) tested green, but it is not the live merge path for `store/substrate-global` (multi-tenant world writers) or `store/projects` (concurrent human+AI). Deployment = issue #73 (ARCH L6); I8's queue hands work to CRDT-merge commits | med |
| G-09 | queue/admission — ⚠ **boilerplate landed (I8, PR #76) but wrong shape for the surface**: `lgwks_admission.py` is in-memory, single-process, **drop-on-full** + admission is **fail-OPEN per-tenant** (RECONCILE.md:318). The real surface is multi-tenant cross-process concurrency (separate crawler proc + many tenant daemons): needs a **durable, per-tenant-fair, no-drop** queue (backpressure not 429). Specced PLANS-NEXT-5.md Step 3 (ARCH L3/L4). 429 path deferred to the external adapter (SCOPE-DEFERRED D1) | high |
