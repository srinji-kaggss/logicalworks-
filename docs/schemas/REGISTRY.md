...
| `lgwks.daemon.event.v1` | 1 | **live** — normalized daemon event envelope for the shared-referee runtime: `tenant_id`, `agent_id`, `session_id`, `actor`, `client`, `lane` (`ingress`/`telemetry`/`workflow`/`control`), `kind` (`human_message`/`transcript_turn`/`tool_call`/`file_change`/`workflow_event`), `scope` (`agent_local`/`shared_referee`), optional `causal_parent_id`, `refs{}`, `payload{}`. Deterministic `event_id` when omitted; same outer contract for Claude/Codex/Gemini adapters | `lgwks_daemon_event.py`; JSON-Schema: `docs/schemas/lgwks.daemon.event.v1.json` |
| `lgwks.daemon.events.query.v0` | 0 | **live, research-grade** — daemon event-log query result: `count`, `items[]` where each item is `lgwks.daemon.event.v1` | `lgwks_daemon_store.py` |
...
| `lgwks.daemon.packet.v0` | 0 | **live (2026-06-12), research-grade** — deterministic session context packet: `schema`, `tenant_id`, `session_id`, `agent_id`, `session_head` (last session-head row or null), `queue` (`lgwks.daemon.queue.v0`), `recent_events[]` (up to N `lgwks.daemon.event.v1` items), `event_count`. Same inputs → same output; used as per-agent subconscious read | `lgwks_daemon_store.py` (`PACKET_SCHEMA`) |
