...
| `lgwks.cortex.index.v1` | 1 | **live** (#238) — CLI result of `state cortex index`: `{ok, session_id, turns, emit_ok, emit_failures, tokenize_failures, transcript}` | `lgwks_cortex.py` (`index_command`) | dict |
| `lgwks.cortex.drop.v1` | 1 | **live** (#247) — stderr loss record emitted when a cortex trajectory emit or tokenize is dropped (best-effort capture, never silent): `{schema, event, session_id, turn_id, error}` | `lgwks_cortex.py` (`_drop_log`) | dict |
| `lgwks.route.act.v1` | 1 | **live** — single agentic front door `route act`: maps NL intent → typed action (`{verb,kind,effect_class,reason}`), executes safe read/network capabilities, blocks mutation intents; `{ok,executed,blocked,block_reason,action,phases[],engine}` | `lgwks_route.py` (`act_intent`, `SCHEMA_ACT`) | dict |
