...
> `scripts/gen_navmap.py` from source — re-run to refresh. **174 modules · 60,246 LOC.** This is the canonical repo map: if someone says "review the map" or "check the navmap", they mean this file unless another map is explicitly named. Read/query this FIRST. Strict machine-readable contract: `docs/navmap/index.json` (`lgwks.navmap.v1`).

**Staleness:** `active` 172 · `staling` 2
...
## Ingestion spine (I1–I12)  ·  17 mod · 7,360 LOC
...
| `lgwks_extract` | ingest every file format → text. The "read anything" port. | 324 | active | test ←6 →6 4d |
...
| `lgwks_pipeline` | unified ingestion and ranking spine. | 1328 | active | cli test ←2 →17 5d |
| `lgwks_promote` | audited tenant→world promotion (ARCH L5, I8-hardening #89). | 146 | active | ←3 →3 9d |
| `lgwks_rank` | cubic node centrality (Z-eigenpair) + AI-discrepancy δ (I6). | 541 | active | cli test ←3 →1 5d |
...
| `scripts.build_capability_idf` | freeze the I8 demand-weight table (stdlib only, no AI). | 77 | active | ←5 →2 9d |

## Research / web acquisition / extract  ·  14 mod · 5,469 LOC

| module | purpose | loc | stale | rel |
|---|---|---|---|---|
| `lgwks_auth_runtime` | read-only auth resolver for crawler fetches. | 225 | active | ←3 5d |
...
| `lgwks_html` | robust, deterministic HTML-to-Markdown and semantic link/table parser. | 352 | active | test ←5 →1 8d |
| `lgwks_jarvis` | legacy deterministic research graph crawler. | 1122 | active | cli ←2 →7 0d |
| `lgwks_public` | open-license public source layer. | 187 | active | cli ←1 5d |
| `lgwks_search` | the missing primitive: a zero-key, free web + news search provider. | 525 | active | test ←6 →4 6d |
...
| `lgwks_sites` | site-aware extractors for high-value platforms. | 195 | active | test ←1 5d |
...
## Graph / AST / code intelligence  ·  9 mod · 6,481 LOC

| module | purpose | loc | stale | rel |
|---|---|---|---|---|
| `graphify.__init__` | — | 4 | active | ←2 11d |
| `lgwks_audit_graph` | U5 Build #5: The Liquid Brain (ADR-sast-003). | 215 | active | test ←1 →2 8d |
| `lgwks_codebase` | semantic codebase database for AI-native code understanding. | 792 | active | cli test ←2 →3 0d |
| `lgwks_entity_graph` | offline document entity graph builder. | 861 | active | cli test ←7 →5 0d |
| `lgwks_graph` | functional, traversable codebase graph with query engine and persistence. | 1619 | active | test ←8 →2 3d |
| `lgwks_graph_viz` | simple localhost graph visualization. | 1207 | active | test ←4 →3 2d |
| `lgwks_refactor` | deterministic AST-based refactoring engine. | 362 | active | cli test ←1 →2 5d |
| `lgwks_repo` | repo lifecycle commands: audit, recover, cleanup, merge, handoff, graph. | 756 | active | cli test ←6 →7 5d |
| `lgwks_review` | graph-aware, spec-bound code review. | 665 | active | cli test ←3 →10 5d |

## Harness / daemon / orchestration  ·  39 mod · 13,368 LOC
...
| `lgwks_context` | graduated-resolution (LOD) context pack for the next spawn (#9 harness layer). | 199 | active | cli ←2 2d |
...
| `lgwks_engine` | U6: Subconscious Engine (deterministic first slice). | 509 | active | cli test ←4 →7 5d |
| `lgwks_ground` | fused live grounding for the research loop (#9 / harness layer). | 144 | active | ←2 →3 4d |
| `lgwks_had` | Human Assumption Decoder (consultant semantic-escalation-harness, intent math). | 454 | active | test ←1 →5 3d |
| `lgwks_hooks` | audit-first hook system for lgwks. (hardened v2) | 891 | active | cli test ←1 →2 7d |
| `lgwks_map` | U1 Capability Map (second-harness PRD §12). | 106 | active | cli ←4 →2 5d |
| `lgwks_portal` | deterministic portal packets for coding-agent re-entry. | 279 | active | cli test ←2 →3 5d |
| `lgwks_project` | one-prompt project orchestrator front door (re-export shim). | 125 | active | cli ←5 →5 6d |
| `lgwks_project_deploy` | `lgwks project deploy` verb. | 470 | active | ←3 →7 3d |
| `lgwks_project_plan` | `lgwks project plan` verb. | 121 | active | ←3 →2 3d |
| `lgwks_project_review` | `lgwks project review` verb. | 122 | active | ←2 →3 3d |
...
| `lgwks_solve` | the first real-world experience: "I have this mess / this thought — prove what happened." | 432 | active | cli test ←5 →5 5d |
| `lgwks_spawn` | AI-AI handoff packet assembler (#9 harness layer). | 207 | active | cli test ←2 →2 12d |
| `lgwks_substrate_run` | build, query, and baseline orchestration for substrate runs. | 841 | active | cli ←5 →11 0d |
...
| `lgwks_workflows` | unified AI workflow harness. | 1210 | active | cli ←2 →18 0d |

## Membrane / intent / steering  ·  10 mod · 3,194 LOC

| module | purpose | loc | stale | rel |
|---|---|---|---|---|
| `lgwks_concept` | deterministic concept extraction and activation steering. | 624 | active | test ←1 →2 5d |
| `lgwks_intent` | schema-driven intent router. A 10-line declaration drives automation. | 554 | active | cli test ←2 →4 0d |
| `lgwks_intent_classifier` | custom English intent classifier for the CLI membrane. | 498 | active | test ←5 →5 3d |
...
| `tools.calibrate_intent_thresholds` | calibrate_intent_thresholds.py — validate and calibrate authority thresholds | 117 | active | ←7 →2 3d |
...
## Governance / gates / refusal / auth  ·  15 mod · 4,826 LOC
...
| `lgwks_embed` | deterministic local folder embedding vault. | 207 | active | cli ←6 →7 5d |
| `lgwks_gate` | unified safety and governance gate router. | 95 | active | cli ←1 →5 5d |
| `lgwks_gate_arch` | G1 Architecture gate (spec-00). | 263 | active | test ←1 →1 15d |
| `lgwks_gate_framework` | G3 Framework-Reality gate (spec-00). | 267 | active | test ←1 →1 6d |
| `lgwks_gate_idiom` | G2 Idiom gate (spec-00). | 150 | active | test ←1 →4 5d |
| `lgwks_keyvault` | macOS Keychain-backed secret resolver for runtime API keys (Issue #7). | 132 | active | cli ←6 12d |
| `lgwks_model_port` | the one runtime gateway every cognition request flows through. | 300 | active | test ←10 →6 5d |
...
| `lgwks_storage` | D4 Three-Syscall Storage Gate (ADR-068). | 880 | active | test ←7 →8 0d |
| `lgwks_urlrisk` | G3 scope curator (Issue #7, ADR-001 §5, constitution L9). | 244 | active | ←1 →1 7d |
...
## CLI / home / membrane surface  ·  5 mod · 3,526 LOC

| module | purpose | loc | stale | rel |
|---|---|---|---|---|
| `lgwks_foundation` | T3 structured extraction via Apple Foundation Models (macOS 26+, on-device). | 200 | active | cli ←2 12d |
| `lgwks_gh` | GitHub surface: issues, PRs, state maps, hardening, deterministic "what's next". | 899 | active | cli test ←2 →4 0d |
...
| `lgwks_ui` | our own terminal visual language. Deliberately NOT Claude Code. | 117 | active | ←19 15d |

## Substrate / storage / schema  ·  14 mod · 3,845 LOC

| module | purpose | loc | stale | rel |
|---|---|---|---|---|
| `lgwks_batch` | schema-validated batch execution for real shell commands. | 288 | active | cli ←1 →2 6d |
| `lgwks_cache` | the UNTRUSTED-CACHE store (build #2, z2 evidence / z4 quarantine). | 114 | active | ←2 →1 7d |
| `lgwks_capture` | unified operator-facing capture compiler over substrate + portal. | 183 | active | cli test ←1 →3 7d |
| `lgwks_cognition` | the COGNITION-LOG store (build #2, z4 core). | 190 | active | ←7 →3 5d |
| `lgwks_memory` | deterministic project memory chain (hardened, build #3). | 272 | active | cli test ←3 →6 5d |
| `lgwks_project_artifacts` | shared schemas, JSONL writers, record builders, | 1159 | active | ←11 →3 3d |
| `lgwks_schema` | schema registry for next-agent discovery. | 283 | active | cli test ←7 8d |
| `lgwks_sqlite` | Shared SQLite connection hardening for lgwks durable stores. | 299 | active | ←13 6d |
| `lgwks_substrate` | thin facade re-exporting all substrate sub-modules. | 223 | active | test ←7 →11 2d |
| `lgwks_substrate_config` | constants, paths, regexes, and shared types for substrate runs. | 109 | active | ←22 6d |
| `lgwks_substrate_io` | file system I/O, JSONL/JSON emission, and manifest loading. | 145 | active | ←12 →2 6d |
| `lgwks_substrate_text` | text processing: chunking, scoring, stemming, fact extraction. | 137 | active | ←5 →1 13d |
...
| module | purpose | loc | stale | rel |
|---|---|---|---|---|
| `lgwks_apple` | Apple-local embedding provider seam. | 146 | active | ←2 14d |
| `lgwks_coreml` | local text classification via CoreML. | 142 | active | ←1 18d |
| `lgwks_jepa` | first executable multi-view JEPA package surface. | 307 | active | cli test ←1 →6 5d |
| `lgwks_model_hub` | repo-resident model loading + developer setup for local CoreML use. | 642 | active | cli test ←4 →3 6d |
| `lgwks_model_mesh` | model law rendered as a single queryable manifest (#119). | 339 | active | test ←4 2d |
...
| `lgwks_openrouter_embed` | optional remote embedding seam via OpenRouter. | 66 | active | test ←1 →3 5d |
| `scripts.build_capability_embeddings` | freeze the Qwen verb-embedding matrix (U6.2 #85). | 102 | active | ←5 →2 9d |
...
## Unclassified (triage)  ·  25 mod · 4,504 LOC

| module | purpose | loc | stale | rel |
|---|---|---|---|---|
| `lgwks_access` | CapabilityPort interface and HMAC impl (#98 / #97 seam). | 518 | active | cli test ←4 →5 6d |
...
| `lgwks_chunking` | canonical text chunking strategies. | 262 | staling | test 0d |
| `lgwks_cli_introspect` | one source of truth for live CLI parser introspection + the | 144 | active | ←2 0d |
| `lgwks_clock` | the single source of truth for timestamps. | 28 | active | ←13 7d |
| `lgwks_config` | validated YAML config surface (lgwks.config.v1). | 133 | active | test ←3 6d |
...
| `lgwks_fabric_reader` | the unified read surface over the State Fabric. | 88 | active | test ←1 →2 6d |
| `lgwks_hashing` | the single source of truth for content hashing. | 88 | active | test ←39 7d |
| `lgwks_inline` | unified payload-inlining resolver. | 136 | active | test ←8 6d |
| `lgwks_jailbreak` | entrypoint injection-risk sensor + abstention verdict. | 174 | active | ←2 →1 5d |
| `lgwks_lexicon` | the one canonical LEXICAL analyzer (word / identifier tokenisation). | 66 | active | ←8 →1 5d |
...
| `lgwks_research` | autonomous deep-research loop (Issue #9, parent #7). | 831 | active | cli ←3 →9 5d |
...
| `lgwks_vecmath` | the single source of truth for vector similarity math. | 82 | active | ←14 5d |
