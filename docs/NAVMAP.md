...
| module | purpose | loc | stale | rel |
|---|---|---|---|---|
| `lgwks_auth_runtime` | read-only auth resolver for crawler fetches. | 200 | active | ←3 2d |
| `lgwks_browser` | bot-resilient, JS-rendering fetch via a real browser (playwright). The eyes for pages | 571 | active | test ←8 →2 1d |
| `lgwks_crawl` | single-page fetch shim: delegates to lgwks_substrate.build_run(max_pages=1). | 204 | active | cli test ←2 →4 2d |
| `lgwks_expression` | - lgwks-expression/1 parser and resolver. | 768 | active | ←1 →1 2d |
...
| module | purpose | loc | stale | rel |
|---|---|---|---|---|
| `axiom.__init__` | the standalone byte framework for the Axiom machine-first ISA. | 20 | active | ←12 2d |
| `axiom.capsule` | The Capsule — one typed, content-addressed record. Claim asserts; Hole abstains. The unit the verifier | 171 | active | ←12 2d |
| `axiom.cid` | Content identity (CID) — the address of a node over its CANONICAL bytes. | 52 | active | ←12 2d |
| `axiom.fabric` | The fabric — immutable content-addressed DAG + hash-chained append-only log + the pending→committed | 144 | active | ←12 2d |
| `axiom.varint` | LEB128 base-128 varints — the lowest byte-layer primitive (WASM uses these for all lengths/indices). | 92 | active | ←12 2d |
| `axiom.verify` | The decidable click — the trust core. A capsule attaches IFF this returns ok. Pure, decidable, 0-AI, | 112 | active | ←12 2d |
...
| module | purpose | loc | stale | rel |
|---|---|---|---|---|
| `lgwks_aup` | AUP runtime gate with Defense-in-Depth. | 710 | active | cli test ←3 →1 2d |
| `lgwks_comprehend` | the Comprehension Gate (spec-01). | 199 | active | cli test ←1 →1 2d |
| `lgwks_embed` | deterministic local folder embedding vault. | 237 | active | cli ←6 →1 2d |
| `lgwks_gate_arch` | G1 Architecture gate (spec-00). | 263 | active | test ←1 →1 2d |
...
| `lgwks_memory` | deterministic project memory chain (hardened, build #3). | 277 | active | cli test ←4 →1 2d |
| `lgwks_project_artifacts` | shared schemas, JSONL writers, record builders, | 1068 | active | ←11 →1 2d |
| `lgwks_schema` | schema registry for next-agent discovery. | 263 | active | cli test ←8 0d |
| `lgwks_sqlite` | Shared SQLite connection hardening for lgwks durable stores. | 276 | active | ←6 2d |
| `lgwks_substrate` | thin facade re-exporting all substrate sub-modules. | 200 | active | test ←7 →12 2d |
| `lgwks_substrate_config` | constants, paths, regexes, and shared types for substrate runs. | 102 | active | ←7 2d |
| `lgwks_substrate_db` | SQLite substrate index DB and global fact vector upserts. | 218 | active | ←2 →2 2d |
| `lgwks_substrate_io` | file system I/O, JSONL/JSON emission, and manifest loading. | 97 | active | ←6 →1 2d |
