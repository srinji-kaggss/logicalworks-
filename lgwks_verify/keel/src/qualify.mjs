#!/usr/bin/env node
// qualify.mjs — DO-330 tool qualification for KEEL ITSELF ("who audits the auditor").
//
// Keel's verdict replaces human review at the floor (docs/00). A tool that gates must PROVE
// it catches what it claims (DO-330 §11.3, TQL-4). The kernel proves its detectors against a
...
const FRONT_ENDS = { profile: 'run.mjs', registry: 'run-registry.mjs' };
...
function main() {
  ...
  console.log('================================================');
  console.log('KEEL TOOL QUALIFICATION — known-bad corpus (DO-330 TQL-4)');
  console.log('================================================');
  if (!fixtures.length) {
  ...
  for (const r of rows)
    console.log(`${r.name.padEnd(30)}${String(r.expect || '').padEnd(10)}${String(r.got || '').padEnd(10)}${r.qualified ? '✓ qualified' : '✗ MISS — ' + r.why}`);

  const misses = rows.filter((r) => !r.qualified);
  console.log('\n================================================');
  if (!misses.length) {
    console.log(`✅ QUALIFIED — Keel returned the correct verdict on all ${rows.length} planted case(s).`);
    process.exit(0);
  }
