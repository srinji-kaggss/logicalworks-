...
_VERB_ALGEBRA: dict[str, dict[str, Any]] = {
    ...
    "batch": {
        "signature": "batch : (spec: BatchSpec, yes: Bool, force: Bool, dry_run: Bool, keep_going: Bool) → ValidationReport × Transcript",
        "domain": {"spec": "lgwks-batch/1 JSON from --file or stdin",
                   "yes": "Bool — approve known-risk batches non-interactively",
                   "force": "Bool — required for destructive batches",
                   "dry_run": "Bool — validate + preview, run nothing",
                   "keep_going": "Bool — continue after a command fails"},
        "codomain": {"report": "ValidationReport", "transcript": "Transcript", "run_dir": "Path"},
        "pre": ["spec.schema == 'lgwks-batch/1'", "every command carries argv + intent + declared risk"],
        "post": ["all commands execute via argv lists (no shell)", "destructive batches require force=True",
                 "validation + transcript persisted under store/batch-runs/ when executed"],
        "side_effects": ["filesystem: writes validation/transcript artifacts", "process: may execute declared commands"],
        "cost": {"time": "O(command_count * command_time)", "tokens": "0", "ram": "O(command_count)"},
        "risk_class": "mutate",
        "inverse": None,
    },
    "project plan": {
