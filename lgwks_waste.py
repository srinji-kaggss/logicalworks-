...
Formula (PRD-04 §04-c):
    used_within_n(item) = item cid/content cited or acted-on within WINDOW_TURNS turns
...
Decisions:
    D1: WINDOW_TURNS = 3 is the pre-registered N — "cited within how many turns?"
        ...
        //why 3: a context item that goes unused across three model turns is unlikely
        to contribute; the window is conservative enough to avoid false-positive waste.
    D2: "cited/acted-on" detection = case-insensitive substring match of the cid string
        against the transcript text within the window. Deterministic and explainable:
        a high-waste injection is attributable to the specific cid that never appeared.
    D3: transcript path is injected as an argument, env-pinnable via
        LGWKS_TRANSCRIPT_PATH — NEVER hardcoded (the dead-space-named-dir trap, PLANS-NEXT-4).
    ...
    D5: ledger is persisted via lgwks_cognition (append-only store) — do NOT mint
        a second log/store (REGISTRY §1 one byte-truth).
"""
...
WINDOW_TURNS: int = 3      # N: cited-within-N-turns window (PRD-04 open-Q, line 93)
...
def _detect_use(cid: str, turns: list[str]) -> tuple[bool, int | None]:
    """Return (used, first_use_turn_index) where used = cid appears in any turn text."""
    needle = cid.lower()
...
def _load_transcript(path: str | Path) -> list[dict]:
    """Load a JSONL transcript file. Returns list of parsed objects; empty on missing/unreadable."""
    p = Path(path)
    if not p.exists():
        return []
    records = []
...
def _extract_turn_texts(transcript: list[dict]) -> list[str]:
    """Extract the text content of each turn from a transcript record list.

    Each record may be a dict with a "content", "text", "message", or "role" key.
    We collect all string-valued leaves for matching.
    """
    texts = []
    for rec in transcript:
        if isinstance(rec, dict):
            # Try common Claude Code transcript fields
...
def build_ledger(
    ...
    *,
    ...
    """Build a lgwks.waste.ledger.v1 dict from a list of lgwks.inbound.v1 packs
    and a transcript (JSONL path or pre-parsed list).
    ...
    window_turns: the pre-registered N (D1); override only in tests.
    ...
    # Collect items from all packs
    items: list[dict] = []
    for pack in packs:
        if pack.get("schema") != "lgwks.inbound.v1":
            continue
        budget = pack.get("budget", {})
        est_map: dict[str, int] = {}

        # handles[] + their scores → tokens estimated from budget
        for h in pack.get("handles", []):
            cid = h if isinstance(h, str) else str(h)
            # est_tokens: use depth_handles lookup, else budget.used_tokens / len(handles)
            est_map[cid] = _est_tokens_for_cid(cid, pack)

        # depth_handles[]
        for dh in pack.get("depth_handles", []):
            cid = dh.get("id", "")
            if cid:
                est_map[cid] = dh.get("est_tokens", 0)

        for cid, tok in est_map.items():
            if not cid:
                continue
            # Determine the injection turn index (use pack position as approximation)
            inject_turn = len(items)
            window_end = min(inject_turn + window_turns, len(turn_texts))
            window = turn_texts[inject_turn:window_end]
...
def _est_tokens_for_cid(cid: str, pack: dict) -> int:
    """Estimate tokens for a cid from the pack's depth_handles (preferred) or budget."""
    for dh in pack.get("depth_handles", []):
        if dh.get("id") == cid:
            return dh.get("est_tokens", 0)
    # Fallback: split budget evenly across handles
    handles = pack.get("handles", [])
    if not handles:
        return 0
    used = pack.get("budget", {}).get("used_tokens", 0)
    per = used // len(handles) if len(handles) > 0 else 0
    return max(per, 0)


def _session_id_from_transcript(records: list[dict]) -> str:
    """Extract session id from transcript records, or return 'unknown'."""
    for rec in records:
        if isinstance(rec, dict):
...
def _waste_rate_from_totals(tokens_injected: int, tokens_used: int) -> float:
    ...
    if tokens_injected <= 0:
        return 0.0
    rate = 1.0 - tokens_used / tokens_injected
...
def worst_item(ledger: dict) -> dict | None:
    """Return the highest-token unused item (the most attributable waste), or None."""
...
def persist_ledger(ledger: dict, *, stream: str = "waste") -> dict:
    """Append the ledger to the cognition log (one byte-truth — D5).

    Logs as kind='note' under stream='waste' (append-only, HMAC-chained).
    Returns the appended cognition entry.
    """
    import lgwks_cognition

    log = lgwks_cognition.CognitionLog(stream)
    # Strip non-serialisable keys before logging
    safe = {k: v for k, v in ledger.items() if k != "items"}
    safe["item_count"] = len(ledger.get("items", []))
    return log.append("note", {"ledger_summary": safe, "schema": SCHEMA})
...
def add_parser(sub) -> None:
    ...
    report_p = sp.add_parser("report", help="compute and print waste ledger from packs + transcript")
    report_p.add_argument("packs", help="path to lgwks.inbound.v1 JSON pack (or JSON array of packs)")
    ...
    report_p.add_argument("--window", type=int, default=WINDOW_TURNS, metavar="N",
                          help=f"citation window turns N (pre-registered default: {WINDOW_TURNS})")
    report_p.add_argument("--persist", action="store_true",
                          help="persist ledger summary to cognition log (lgwks_cognition)")
...
def _cmd_report(args) -> int:
    ...
    try:
        ...
        print(f"error: cannot load packs file: {e}", file=_sys.stderr)
        return 1
    ...
    if rate > SUGGEST_CUT_THRESHOLD:
        print(f"  NOTE: waste_rate {rate:.3f} > threshold {SUGGEST_CUT_THRESHOLD} "
...
def _cmd_info(args) -> int:
    print(json.dumps({
        ...
        "detection": "cid substring match against transcript within window_turns (deterministic)",
        "scope_fence": "measurement only — I11 REPORTS threshold breach, does NOT act on it",
        "clock": "no wall-clock in build_ledger — same (packs, transcript, N) → identical ledger",
    }, indent=2))
    return 0
